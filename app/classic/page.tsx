...
import { Check, Heart, Search, BookOpen, Trophy, Flag, Info, ChevronDown, SlidersHorizontal, X } from "lucide-react"
...
import { ALLY_TIPS as allyTips } from "@/lib/ally-tips"
import { QUIZ_QUESTIONS as quizQuestions } from "@/lib/quiz-questions"
                ...
                return (
                  ...
                  >
                    <Button
                      ...
                      size="lg"
                      className={cn(
                        "quiz-answer-btn h-auto min-h-12 w-full justify-start rounded-md px-4 py-3.5 text-base font-semibold leading-snug whitespace-normal text-pretty",
                    ...
                    >
                      <span
                        className={cn(
                          ...
                          isCorrect || isWrongSelection ? "items-start gap-3" : "",
                        )}
                      >
                        {isCorrect ? (
                          <Check className="mt-0.5 size-5 shrink-0 text-primary" strokeWidth={2.75} aria-hidden />
                        ) : null}
                        {isWrongSelection ? (
                          <X className="mt-0.5 size-5 shrink-0 text-destructive" strokeWidth={2.75} aria-hidden />
                        ) : null}
                        <span className={cn("min-w-0 text-pretty", (isCorrect || isWrongSelection) && "flex-1")}>
                          {option}
                        </span>
                      </span>
                    </Button>
